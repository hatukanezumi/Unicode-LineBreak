...
KEY => VALUE の対については L<オプション in Unicode::LineBreak|POD2::JA::Unicode::LineBreak/オプション>を参照。
第二の形式では、
L<Unicode::LineBreak> オブジェクト LINEBREAK で分節の仕様を決定する。
...
詳しくは L<DESCRIPTION in Unicode::LineBreak|POD2::JA::Unicode::LineBreak/DESCRIPTION> を参照。
...
(L<Unicode::LineBreak|POD2::JA::Unicode::LineBreak> 参照) を返す。
OFFSET を指定しないと、次の位置の書記素クラスタの情報を返す。
