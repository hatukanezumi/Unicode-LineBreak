...
I<Instance method>.
If MAX is not specified, calculate I<number of columns> of Unicode string
PRE.SPC.STR based on character widths defined by [UAX #11].
If a positive value MAX is specified, return I<number of characters> of SUBSTR
that is the longest substring of STR by which number of columns of
...
By default, getstrsize() is used to calculate string sizes.
