...
Returns an array of three elements (LEN, COLS, CLASS).
LEN is number of Unicode characters grapheme cluster contains, including prepending character(s).
COLS is number of columns of grapheme cluster determined by built-in character database.
CLASS is line breaking property (class) of the first Unicode character of grapheme base, except that SA being resolved to AL or CM.
...
Following characters succeeding character with line breaking property CB
behave as if they were CM, because of their Grapheme_Cluster_Break properties.

    UCS    |Name                                     |LBC|GBC
    -----------------------------------------------------------------
    U+0F7F |TIBETAN SIGN RNAM BCAD                   |BA |SpacingMark
    U+FF9E |HALFWIDTH KATAKANA VOICED SOUND MARK     |NS |Extend
    U+FF9F |HALFWIDTH KATAKANA SEMI-VOICED SOUND MARK|NS |Extend
    -----------------------------------------------------------------
...
if they have not been assigned any characers.
    ...
    U+E000..U+F8FF     | AL (XX)    | A          | Private use
    U+F900..U+FAFF     | ID         | W          | CJK ideographs
    U+20000..U+2FFFD   | ID         | W          | CJK ideographs
    U+30000..U+3FFFD   | ID         | W          | CJK ideographs
    U+F0000..U+FFFFD   | AL (XX)    | A          | Private use
    U+100000..U+10FFFD | AL (XX)    | A          | Private use
