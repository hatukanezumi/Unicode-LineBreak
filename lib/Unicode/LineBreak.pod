...
appropriate classes. 
However, when word segmentation for South East Asian scripts is enabled,
characters of supported scripts (currently Thai only) will be kept SA.
...
=item Unicode::LineBreak::SouthEastAsian::supported

Flag to determin if word segmentation for South East Asian scripts is enabled.
If this feature was enabled, a non-empty string is set. 
Otherwise, C<undef> is set.

B<N.B.>: Current release supports Thai script of Thai language only.
...
When word segmentation for South East Asian languages is not supported,
