...
3 つの要素を持つ配列 (LEN, COLS, CLASS) を返す。
LEN は書記素クラスタに含まれるUnicode文字の数。
COLS は書記素クラスタの桁数 (組み込みの文字データベースで決定される値)。
CLASS は書記素基底〔grapheme base〕の最初のUnicode文字の行分割特性 (分類)、ただし SA の場合は AL か CM に解決する。
...
以下の文字は、行分割特性 CB の文字の後に続くとき、Grapheme_Cluster_Break 特性により、CM のようにふるまう。
