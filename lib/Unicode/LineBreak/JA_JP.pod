...
これらの用語は文字学の観点からは不正確である。音素文字的な文字には「アルファベット」に分類されない文字も含む。表語文字的な文字には漢字以外の文字、さらには一部の表音文字体系の文字をも含む。
...
I<インスタンスメソッド>。
Unicode 文字列 STRING の最初の文字の行分割特性を得る。
...
B<注>:
このメソッドは
BK、CR、LF、NL、SP、CM の各分類に関わる規則については適切な値を返せない。
...
ハングル音節とハングル連結チャモ〔conjoining jamo〕を音素文字的な文字 (AL) に扱う。
初期値は C<"NO">。
...
長さは整数でなくてもよい。長さの単位は随意に選べるが、MaxColumns オプションのそれと一致させなければならない。
