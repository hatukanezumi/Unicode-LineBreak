...
以下の文字が、行分割特性が CM であるかのようにふるまう。
    ...
    U+0F7F |チベット文字ナムチャド〔ヴィサルガ〕|BA |SpacingMark
    U+FF9E |濁点 (代替名称)                     |NS |Extend
    U+FF9F |半濁点 (代替名称)                   |NS |Extend
    ------------------------------------------------------------
...
以下の UCS の範囲にあるコードポイントは、文字が割り当てられていなくても決まった特性値を持つ。
    ...
    U+E000..U+F8FF     | AL (XX)    | A          | 私用領域
    U+F900..U+FAFF     | ID         | W          | CJK漢字
    U+20000..U+2FFFD   | ID         | W          | CJK漢字
    U+30000..U+3FFFD   | ID         | W          | CJK漢字
    U+F0000..U+FFFFD   | AL (XX)    | A          | 私用領域
    U+100000..U+10FFFD | AL (XX)    | A          | 私用領域
