...
I<インスタンスメソッド>。
Unicode 文字列 STR の位置 POS から始まるB<書記素クラスタ>〔grapheme cluster〕の情報を得る。
書記素クラスタについて詳しくは [UAX #29] を参照。

3 つの要素を持つ配列 (CLASS, GLEN, ELEN) を返す。
CLASS は書記素クラスタの最初の文字の行分割特性 (分類)、ただし SA の場合は AL か CM に解決する。GLEN は書記素基底〔grapheme base〕(プリペンド文字を含む) の文字数。ELEN は書記素エキステンダ〔grapheme extender〕の文字数。

B<注>: [UAX #29] では仮名の濁点と半濁点 (ともに合成可能でないもの) が書記素エキステンダ (の一部) になりうるとしているが、当モジュールでは書記素基底と扱う。
...
L</TailorLB> オプションおよび L</TailorEA> オプションで個々の文字の行分割特性 (分類) や East_Asian_Width 特性を調整できる。その際に便利な定数をいくつか定義してある。

=head3 行分割特性

初期値では、若干の平仮名や片仮名や仮名に準ずるものを行頭禁則文字 (NS) と扱う。
以下の対を L<TailorLB> オプションに指定すれば、これらの文字を通常の表語文字的な文字 (ID) と扱う。
...
=item C<AMBIGUOUS_ALPHABETIC() =E<gt> EA_N>
...
文字の分類と行分割の基本的な規則は L<Unicode::LineBreak::Rules> で定義している。
これをカスタマイズしたい場合は、ソースパッケージの F<data> ディレクトリの中を見ること。
...
ただし、Grapheme_Cluster_Break 特性の値が Extended か SpacingMark である文字は CM に解決する。
