...
B<注>:
このメソッドは、C<"BK">、C<"CR">、C<"LF">、C<"NL">、C<"SP">、C<"CM"> の各分類に関わる規則については適切な値を返せないかもしれない。
...
任意分割によって生じる行が MinColumns や MaxColumns の限度を超えると見込まれるときは、引き続く文字列に対してB<緊急分割>を実行できる。
L</UrgentBreaking> オプションを参照。
...
C<Format> オプションにサブルーチンへの参照を指定する場合、そのサブルーチンは 3 つの引数を取らなければならない。オブジェクトのインスタンス、事象の種類、文字列。
...
C<UrgentBreaking> オプションにサブルーチンへの参照を指定する場合、そのサブルーチンは 5 つの引数を取らなければならない。オブジェクトのインスタンス、先立つ行の長さ (LEN とする)、その行の Unicode 文字列 (PRE)、追加される空白文字 (SPC)、分割すべき Unicode 文字列 (STR)。
サブルーチンは、文字列 STR を分割した結果の配列を返さなければならない。

C<UserBreaking> オプションの要素に [REGEX, SUBREF] の配列参照を指定する場合、サブルーチンは 2 つの引数を取らなければならない。オブジェクトのインスタンス、文字列。
サブルーチンは、文字列を分割した結果の配列を返さなければならない。


=head3 文字列長の算出

C<SizingMethod> オプションにサブルーチンへの参照を指定する場合、そのサブルーチンは 5 つの引数を取らなければならない。オブジェクトのインスタンス、元の文字列の長さ (LEN とする)、元の Unicode 文字列 (PRE)、追加される空白文字 (SPC)、追加される Unicode 文字列 (STR)。

サブルーチンは C<PRE.SPC.STR> の長さを算出して返さなければならない。
長さは整数でなくてもよい。長さの単位は随意に選べるが、L</MinColumns> オプションおよび L</MaxColumns> オプションのそれと一致させなければならない。
