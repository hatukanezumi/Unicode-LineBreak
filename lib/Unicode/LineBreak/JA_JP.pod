...
B<注>:
このメソッドは、行分割動作のおおまかな動作を表す値を返すにすぎない。
...
[UAX #14] で定義される 38 の行分割特性 (分類) を表す値。

B<注>:
特性値 CP はUnicode 5.2.0版で導入された。
特性値 HL はUnicode 6.1.0版で導入されそう。
...
このモジュールで用いている文字の特性値は、Unicode 標準 6.1.0ベータ版 (2011-09-19現在) による。
