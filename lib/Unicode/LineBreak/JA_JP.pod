...
EA_Na、EA_N、EA_W、EA_H、EA_F、EA_Z のいずれかの定数が返る。
...
ただし、東南アジアの用字系のための単語分節が有効になっているときは、対応している用字系 (現在はタイ文字のみ) の文字の特性を SA のまま解決しない。
...
東南アジアの用字系のための単語分節機能が有効かどうかを示すフラグ。
